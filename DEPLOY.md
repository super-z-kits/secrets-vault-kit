...
Capture `token.key` — that's what you put in the deployment target's secret store. Save it to a `chmod 600` file (not a shell var — env vars leak to `/proc/<pid>/environ`).
